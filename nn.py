...
class ResNet18(nn.Module):
    def __init__(self, block, class_size: int = 6):
        ...
        self.fc = nn.Linear(512 * block.expansion, class_size)
...
def validate(net, valid_loader):
    ...
    with torch.no_grad():
        ...
        val_loss = nn.CrossEntropyLoss(preds, trues)
...
def main():
    ...
    ax1.plot(np.arange(EPOCH), log['train_loss'], label='train_loss')
    ax1.plot(np.arange(EPOCH), log['valid_loss'], label='valid_loss')
    ax1.set_xlabel('epoch')
    ax1.set_ylabel('loss')
    ax1.set_title('Loss')

    ax2 = fig.add_subplot(212)
    ax2.plot(np.arange(EPOCH), log['train_err'], label='train_err')
    ax2.plot(np.arange(EPOCH), log['valid_err'], label='valid_err')
    ax2.set_xlabel('epoch')
    ax2.set_ylabel('error rate')
    ax2.set_title('Error rate')

    fig.savefig(f'./image_out/{datetime.today().isoformat().replace(":", "-").split("T")[0]}_nn.eps', format='eps')
